...
export default function Welcome({ auth = {}, articles = [], schedules = [], balance = 0 }) {
    ...
    // --- CARI JADWAL TARAWIH DARI DATABASE ---
    // Mencari agenda yang judulnya mengandung kata 'tarawih' (case insensitive)
    const tarawihSchedule = schedules.find(s => 
        s.title.toLowerCase().includes('tarawih') || 
        s.type === 'tarawih'
    );

    // Helper untuk data terbaru lainnya
    ...
    const calculateIftarCountdown = (jadwal) => {
        ...
        if (now < maghribTime) {
            ...
            setTimeToMaghrib(`-${hours}j ${minutes}m menuju Ifthar`);
        } else {
            setTimeToMaghrib("Waktunya Berbuka");
        }
    };
    ...
    const determineNextPrayer = (jadwal) => {
        ...
        // Jika ada Tarawih dari database, masukkan ke logika penentuan waktu berikutnya
        if (tarawihSchedule) {
            // Asumsi format time dari DB adalah "HH:MM:SS" -> ambil "HH:MM"
            const tTime = tarawihSchedule.time.substring(0, 5); 
            times.push({ id: 'tarawih', time: tTime });
        }
    ...
    // --- LIST KARTU JADWAL SHOLAT ---
    const prayerTimesList = [
        { id: 'imsak', label: 'Imsak', icon: <Moon size={18}/> },
        { id: 'subuh', label: 'Subuh', icon: <Sunrise size={18}/> },
        { id: 'dzuhur', label: 'Dzuhur', icon: <Clock size={18}/> },
        { id: 'ashar', label: 'Ashar', icon: <Clock size={18}/> },
        { id: 'maghrib', label: 'Maghrib', icon: <Sunset size={18}/> },
        { id: 'isya', label: 'Isya', icon: <Moon size={18}/> },
    ];

    // Jika ada jadwal tarawih di DB, tambahkan ke list tampilan
    if (tarawihSchedule) {
        prayerTimesList.push({ 
            id: 'tarawih', 
            label: 'Tarawih', 
            icon: <Star size={18}/>, // Icon Bintang untuk Tarawih
            customTime: tarawihSchedule.time.substring(0, 5) // Waktu manual dari DB
        });
    }
    ...
    return (
        <div className="bg-[#0f172a] min-h-screen font-sans text-slate-200 antialiased selection:bg-amber-500 selection:text-white overflow-x-hidden">
            ...
            {/* --- JADWAL SHOLAT (DENGAN TARAWIH DINAMIS) --- */}
            <section id="jadwal" className="px-4 mb-24 -mt-10 relative z-20">
                <div className="max-w-7xl mx-auto bg-gradient-to-b from-[#1e293b] to-[#0f172a] rounded-[3rem] relative overflow-hidden shadow-2xl shadow-black/50 border border-white/5">
                    
                    {/* Background Mosque Silhouette */}
                    <div className="absolute bottom-0 left-0 right-0 h-48 md:h-64 z-0 pointer-events-none opacity-20">
                        <svg viewBox="0 0 1440 320" className="w-full h-full text-amber-900 fill-current">
                        ...
                        <div className="flex flex-col lg:flex-row justify-between items-end mb-16 gap-8">
                            <div className="w-full lg:w-1/2">
                                <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-full mb-4">
                                    ...
                                    <span className="text-xs font-bold text-amber-200 uppercase tracking-wider">Malang & Sekitarnya</span>
                                </div>
                                <h2 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-white leading-tight">
                                    Jadwal Sholat &<br/>
                                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-amber-500">Imsakiyah.</span>
                                </h2>
                            </div>
                            <div className="w-full lg:w-1/2 flex flex-col items-start lg:items-end">
                                {timeToMaghrib && (
                                    <div className="mb-4 inline-flex items-center gap-2 bg-emerald-900/40 border border-emerald-500/30 px-4 py-2 rounded-xl backdrop-blur-md">
                                        <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></div>
                                        <span className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{timeToMaghrib}</span>
                                    </div>
                                )}
                                <div className="flex items-baseline gap-2">
                                    <p className="text-6xl md:text-7xl font-mono font-bold text-white tracking-tighter drop-shadow-lg">
                                        {currentTime.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                                    </p>
                                    <span className="text-xl text-slate-500 font-bold">WIB</span>
                                </div>
                                <p className="text-slate-400 font-medium mt-1 flex items-center gap-2">
                                    <Calendar size={16}/> {currentTime.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                                </p>
                            </div>
                        </div>

                        {/* GRID DENGAN JUMLAH KOLOM DINAMIS (TERGANTUNG ADA TARAWIH/TIDAK) */}
                        <div className={`grid grid-cols-2 md:grid-cols-3 ${tarawihSchedule ? 'lg:grid-cols-7' : 'lg:grid-cols-6'} gap-4`}>
                            {prayerTimesList.map((item) => {
                                const isActive = nextPrayer === item.id;
                                const isMaghrib = item.id === 'maghrib';
                                const isImsak = item.id === 'imsak';
                                const isTarawih = item.id === 'tarawih';
                                
                                return (
                                    <div key={item.id} className={`group relative p-6 rounded-3xl flex flex-col items-center justify-between min-h-[160px] overflow-hidden transition-all duration-500 cursor-default ${isActive 
                                        ? 'bg-gradient-to-b from-amber-500 to-amber-600 shadow-[0_0_40px_rgba(245,158,11,0.4)] scale-105 z-10 border border-amber-400' 
                                        : isTarawih 
                                            ? 'bg-emerald-900/20 border border-emerald-500/20 hover:border-emerald-500/40' // Style khusus kartu Tarawih
                                            : 'bg-white/5 backdrop-blur-sm border border-white/5 hover:bg-white/10 hover:border-white/10'
                                    }`}>
                                        
                                        {/* Glow Effect */}
                                        {isActive && <div className="absolute inset-0 bg-white/20 animate-pulse rounded-3xl"></div>}

                                        {/* Icon */}
                                        <div className={`relative z-10 p-3 rounded-full mb-2 transition-colors ${isActive ? 'bg-white/20 text-white' : isTarawih ? 'text-emerald-400 bg-emerald-500/10' : 'bg-white/5 text-slate-400 group-hover:text-white'}`}>
                                            {isMaghrib && !isActive ? <Utensils size={20}/> : item.icon}
                                        </div>
                                        
                                        {/* Label & Time */}
                                        <div className="relative z-10 text-center">
                                            <p className={`text-xs font-bold uppercase tracking-widest mb-1 ${isActive ? 'text-amber-100' : isTarawih ? 'text-emerald-300' : 'text-slate-500 group-hover:text-slate-300'}`}>
                                                {item.label}
                                            </p>
                                            <p className={`text-2xl font-bold tracking-tight ${isActive ? 'text-white' : isTarawih ? 'text-emerald-100' : 'text-white'}`}>
                                                {/* Tampilkan Waktu Custom (Tarawih) atau dari API */}
                                                {item.customTime 
                                                    ? item.customTime 
                                                    : (loading ? <span className="text-sm opacity-50">...</span> : (jadwalSholat ? jadwalSholat[item.id] : '--:--'))
                                                }
                                            </p>
                                        </div>

                                        {/* Badges */}
                                        {(isMaghrib || isImsak) && !isActive && (
                                        ...
                                        {isActive && (
                                            <div className="absolute bottom-0 inset-x-0 h-1 bg-white/30"></div>
                                        )}
                                    </div>
                                )
                            })}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-6 h-auto md:h-[650px]">
                    ...
                    <div className="md:col-span-1 bg-[#1e293b] text-white border border-white/5 rounded-[2.5rem] p-8 shadow-xl flex flex-col justify-between group cursor-pointer hover:border-amber-500/50 transition-colors relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex justify-between items-start">
                            ...
                            <span className="text-[10px] font-bold bg-amber-500 text-slate-900 px-2 py-1 rounded uppercase">
                                {latestSchedule ? 'Terbaru' : 'Agenda'}
                            </span>
                        </div>
                        <div className="relative z-10 mt-4">
                            {latestSchedule ? (
                                <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                                    <p className="text-xs text-amber-400 mb-1 font-bold uppercase tracking-wider flex items-center gap-1">
                                        <Clock size={12} /> {new Date(latestSchedule.date).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'short' })}
                                    </p>
                                    <h3 className="text-xl font-bold leading-tight line-clamp-3 group-hover:text-amber-400 transition-colors mb-2">
                                        {latestSchedule.title}
                                    </h3>
                                    <p className="text-xs text-slate-400 line-clamp-1">{latestSchedule.ustadz_name || 'Masjid Sabilillah'}</p>
                                </div>
                            ) : (
                                <>
                    ...
                    {/* CARD 4: BERITA (DYNAMIC) */}
                    <div className="md:col-span-1 bg-[#1e293b] border border-white/5 rounded-[2.5rem] shadow-lg flex flex-col justify-between group cursor-pointer hover:border-emerald-500/50 transition-all relative overflow-hidden">
                        <Link href={route('articles.index')} className="absolute inset-0 z-20"></Link>
                        {latestArticle ? (
                            <>
                                <img src={`/storage/${latestArticle.image}`} alt={latestArticle.title} className="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:opacity-50 transition-opacity duration-500" />
                                <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a] via-[#0f172a]/50 to-transparent"></div>
                                <div className="relative z-10 h-full flex flex-col justify-between p-8">
                                    <div className="flex justify-between items-start">
                                        <span className="text-[10px] font-bold bg-emerald-500 text-slate-900 px-2 py-1 rounded uppercase">Update</span>
                                    </div>
                                    <div>
                                        <p className="text-xs text-emerald-400 mb-1 font-bold uppercase tracking-wider">
                                            {new Date(latestArticle.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                                        </p>
                                        <h3 className="text-lg font-bold leading-snug text-white line-clamp-3 group-hover:text-emerald-300 transition-colors">
                                            {latestArticle.title}
                                        </h3>
                                    </div>
                                </div>
