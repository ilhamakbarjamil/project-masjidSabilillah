...
export default function Index({ auth, donations = [], my_donations = [] }) {
    ...
    // --- DATA LOGO BANK & QRIS (UPDATED BNI) ---
    const paymentMethods = [
        ...
        { 
            id: 'bni', 
            name: 'BNI', 
            // Menggunakan URL PNG yang lebih stabil untuk BNI
    ...
    // --- LOGIKA AUTO POLLING (CEK STATUS OTOMATIS) ---
    useEffect(() => {
        let interval;
        if (paymentResult && paymentResult.status === 'pending') {
            interval = setInterval(async () => {
                try {
                    const res = await axios.get(`/infaq/check/${paymentResult.id}`);
            ...
            <div className="py-12 bg-slate-50 min-h-screen font-sans relative">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-10 text-center max-w-2xl mx-auto">
                        <span className="inline-block py-1 px-3 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold uppercase tracking-widest mb-3">
                            Sabilillah Digital
                        </span>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div className="lg:col-span-7 space-y-8">
                            <div className="bg-white p-8 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-3 mb-8 relative z-10">
                                    ...
                                    <h3 className="text-xl font-bold text-slate-900">Formulir Infaq</h3>
                                </div>
                                ...
                                <form onSubmit={handlePay} className="space-y-8 relative z-10">
                                    ...
                                    {/* Payment Methods Grid (LOGO ASLI) */}
                                    <div className="space-y-3">
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                            {paymentMethods.map((method) => (
                                                <div key={method.id} onClick={() => setData('bank', method.id)}
                                                    className={`relative overflow-hidden cursor-pointer h-20 rounded-2xl border flex flex-col items-center justify-center gap-1 transition-all duration-200 group ${
                                                        data.bank === method.id 
                                                        ? 'border-emerald-500 bg-emerald-50/50 ring-2 ring-emerald-500 ring-offset-2' 
                                                        : 'border-slate-100 bg-white hover:border-emerald-200 hover:shadow-md'
                                                    }`}>
                                                    
                                                    {/* Logo Image */}
                                                    <img src={method.logo} alt={method.name} className="h-6 w-auto object-contain mb-1" />
                                                    
                                                    {/* Badge Check if Selected */}
                                                    {data.bank === method.id && (
                                    ...
                                    <button 
                                        ...
                                        className="w-full bg-slate-900 hover:bg-emerald-600 text-white font-bold py-5 rounded-2xl transition-all duration-300 shadow-xl hover:shadow-emerald-200 hover:-translate-y-1 flex items-center justify-center gap-2 uppercase text-xs tracking-[0.2em] disabled:opacity-70 disabled:cursor-not-allowed"
                                    >
                                        {isLoading || processing ? <><Loader2 className="animate-spin" size={16}/> MEMPROSES...</> : <>LANJUTKAN PEMBAYARAN <ArrowRight size={16} /></>}
                                    </button>
                                </form>
                                
                                <div className="mt-6 flex items-center justify-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-wide">
                            ...
                            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                                ...
                                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                    {my_donations.length > 0 ? my_donations.map((item) => (
                                        <div key={item.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex justify-between items-center hover:bg-emerald-50/50 transition">
                                            <div>
                                                <p className="font-black text-slate-700 text-sm">{formatIDR(item.amount)}</p>
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{new Date(item.created_at).toLocaleDateString()}</p>
                                            </div>
                                            <div className={`p-2 rounded-full ${item.status === 'success' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
                                                {item.status === 'success' ? <CheckCircle2 size={14} /> : <Clock size={14} />}
                                            </div>
                                        </div>
                                    )) : (
                                        <p className="text-center text-slate-400 text-xs py-4 italic">Belum ada riwayat infaq.</p>
                                    )}
                                </div>
                            </div>

                            {/* Donatur Terbaru (Social Proof) */}
                                    ...
                                    {donations.slice(0,5).map((item) => (
                                        <div key={item.id} className="flex items-center justify-between border-b border-white/10 pb-3 last:border-0 last:pb-0 animate-in slide-in-from-right duration-500">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-bold text-sm text-white">{item.donor_name}</p>
                    ...
                    {paymentResult && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
                            <div className="bg-white rounded-[2.5rem] max-w-md w-full p-8 relative shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden">
                                ...
                                <button onClick={() => setPaymentResult(null)} className="absolute top-5 right-5 p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition text-slate-500 hover:text-red-500 z-10">
                                    <X size={20} />
                                </button>
                                ...
                                <div className="text-center relative z-10">
                                    ...
                                    <h3 className="text-2xl font-black text-slate-800 tracking-tight">Selesaikan Pembayaran</h3>
                                    <p className="text-slate-500 text-sm mt-1">Total yang harus dibayar:</p>
                                    <p className="text-4xl font-black text-emerald-600 my-4 tracking-tighter">{formatIDR(paymentResult.gross_amount || paymentResult.amount)}</p>
                                        ...
                                        {paymentResult.payment_type === 'qris' && (
                                            <div className="space-y-4">
                                                <div className="bg-white p-2 rounded-xl shadow-sm border border-slate-100 inline-block">
                                                    {/* Ambil URL QR dari actions array Midtrans Core API */}
                                                    <img src={paymentResult.actions?.[0]?.url || paymentResult.payment_code} className="w-48 h-48 object-contain" alt="QRIS Code" />
                                                </div>
                                                <div className="flex items-center justify-center gap-2">
                                                    <img src="https://upload.wikimedia.org/wikipedia/commons/a/a2/Logo_QRIS.svg" className="h-4" alt="QRIS" />
                                                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Scan & Bayar</p>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {paymentResult.payment_type === 'bank_transfer' && (
                                            <div className="space-y-4">
                                                <div className="flex flex-col items-center">
                                                    <img 
                                                        src={paymentMethods.find(p => p.id === paymentResult.va_numbers[0].bank)?.logo} 
                                                        alt={paymentResult.va_numbers[0].bank}
                                                ...
                                                <div className="flex items-center justify-center gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm cursor-pointer hover:border-emerald-500 transition group"
                                                    onClick={() => { navigator.clipboard.writeText(paymentResult.va_numbers[0].va_number); alert("Disalin!"); }}>
                                                    <span className="text-2xl font-mono font-bold text-slate-800 tracking-widest">{paymentResult.va_numbers[0].va_number}</span>
                                                    <Copy size={18} className="text-emerald-500 group-hover:scale-110 transition"/>
                                                </div>
                                    ...
                                    <button onClick={() => window.location.reload()} className="w-full mt-6 bg-slate-900 text-white font-bold py-4 rounded-xl hover:bg-emerald-600 transition shadow-lg shadow-emerald-200">
                                        Saya Sudah Bayar
                                    </button>
                                </div>
                            </div>
